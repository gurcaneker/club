...
- Redis (kuyruk), S3 uyumlu depolama (geliştirmede MinIO)
...
| --- | --- | --- |
| `architect` | Veri modeli, API sözleşmeleri, ADR'ler, görev kırılımı; `packages/shared` tipleri, zod şemaları ve alt modüllerin public API'si | `docs/`, `packages/db/prisma/`, `packages/shared/` (aşağıdaki iki alt dizinin uygulama dosyaları hariç) |
| `backend-dev` | NestJS API, auth, tenant izolasyonu, rıza API'si, `EmailSender` adaptörü, taslak durum geçiş fonksiyonu | `apps/api/`, `packages/shared/src/state/` (uygulama) |
...
| `devops` | Docker Compose, Nginx, CI, ortam değişkenleri | kök yapılandırma, `infra/`, `.github/` |
| `test-engineer` | Tüm test katmanları, test çalıştırma, hata raporu; sentetik fikstür üretimi (tek sahip) | `**/*.test.ts`, `**/*.spec.ts`, `e2e/`, `test/fixtures/` |
